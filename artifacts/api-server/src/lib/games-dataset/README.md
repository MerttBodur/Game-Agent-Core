...
- `engine` - string, prefer names from `toolCatalog.json` where applicable (for example: `Unity`, `Unreal Engine`, `Godot`, `GameMaker`, `Phaser`, `Cocos Creator`, `Defold`, `LOVE`, `Construct 3`, `Bevy`, `Custom`)
- `language` - string, prefer names from `toolCatalog.json` where applicable (for example: `C# with .NET`, `C++`, `GDScript`, `Lua`, `JavaScript / TypeScript`, `Rust`, `Haxe`, `Python`, `Custom`)
...
For popularity merging, keep tool-name spelling aligned with `toolCatalog.json` where possible.
